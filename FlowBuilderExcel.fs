...
let loadSheets (path:string) =
    ...
    flow, supply, supplyTrade, trade, transport, consume
...
let supplyTradeFromRow (sheet: SupplyTradeSheet) : Map<string, SupplyTradeParams> =
     sheet.Data
     ...
     |> Seq.map (fun row ->
            ...
            let p : SupplyTradeParams = {supply = sp; trade = tp}
            p.supply.tcId, p)
     |> Map.ofSeq    


let buildBlocksFromExcel (path:string) : Block list =
    let flowSheet, supplySheet, supplyTrade, tradeSheet, transportSheet, consumeSheet =
        loadSheets path
    ...
    |> Seq.map (fun row ->
        ...
        | "MultiSupplyTrade" ->
            let p = supplyTrade |> Map.filter (fun k _ -> k = row.Ref) |> Map.values
            let multiSupplyTrade = { legs = p |> Seq.toList }
            MultiSupplyTrade multiSupplyTrade
