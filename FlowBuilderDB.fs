...
let buildTradesDB diaGas idFlowMaster path segmentosCompra : Result<Map<flowId, TradeParams>, DomainError> =
    ...
    | Some flowMaster ->
        ...
        let dTransGas = transaccionesGasById().Value
        let dTradeByFlowDetail = tradeByFlowDetailId(diaGas).Value
...
let buildSleevesDB diaGas idFlowMaster path segmentosCompra : Result<Map<flowId, SleeveParams>, DomainError> =
    ...
    | Some flowMaster ->
        ...
        let dSleeve = sleeveByFlowDetailId(diaGas).Value
...
let buildTransportsDB diaGas idFlowMaster path segmentosCompra : Result<Map<flowId, TransportParams>, DomainError> =
  ...
  | Some flowMaster ->
      ...
      let transpFlow = transportByFlowDetailId(diaGas).Value
      let dRuta      = rutaById()
...
let buildFlowStepsDb (flowMasterId: FlowMasterId) (path: string) (diaGas: DateOnly) : Result<FlowStep list, DomainError> =
    ...
    let buildBlock (tipoDesc: string) (fd: FlowDetail) : Result<Block option, DomainError> =
        ...
        | "Trade" ->
            trades |> Result.bind (fun m ->
                match Map.tryFind fd.IdFlowDetail m with
                | Some tp -> Ok (Some (Trade tp))
                | None    -> Ok None)

        | "Transport" ->
            transports |> Result.bind (fun m ->
                match Map.tryFind fd.IdFlowDetail m with
                | Some tp -> Ok (Some (Transport tp))
                | None    -> Ok None)

        | "Sleeve" ->
            sleeves |> Result.bind (fun m ->
                match Map.tryFind fd.IdFlowDetail m with
                | Some sl -> Ok (Some (Sleeve sl))
                | None    -> Ok None)

        | "Sell" ->
            tryFindOr [] fd.IdFlowDetail sells
            |> Result.map (Some << SellMany)

        | "Consume" ->
            consumes |> Result.bind (fun m ->
                match Map.tryFind fd.IdFlowDetail m with
                | Some cp -> Ok (Some (Consume cp))
    ...
    |> List.map (fun fd ->
        ...
        |> Result.map (Option.map (fun block ->
